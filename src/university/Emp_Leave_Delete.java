...
public class Emp_Leave_Delete extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Emp_Leave_Delete frame = new Emp_Leave_Delete();
	...
	public Emp_Leave_Delete() {
		setTitle("Delete Employe Leave");
		setIconImage(Toolkit.getDefaultToolkit().getImage(Emp_Leave_Delete.class.getResource("/Images/uniLogo.png")));
		...
		backBtn.setIcon(new ImageIcon(Emp_Leave_Delete.class.getResource("/Images/backSmall.png")));
		...
		lblEmpId.setBounds(273, 114, 61, 21);
		...
		lblRollNoA.setBounds(336, 114, 75, 21);
	...
	private void deleteData() {
		try {
			String query = "delete from employe_leave where l_id = '" + comboID.getSelectedItem() + "'";
			con.statement.executeUpdate(query);
			JOptionPane.showMessageDialog(null, "Employee Leave Deleted Successfully!");
			setVisible(false);
	...
	private void setDataIntoLabel() {
		try {
			String query = "select date,duration,emp_id from employe_leave where l_id = '"
					+ comboID.getSelectedItem() + "'";
			ResultSet rs = con.statement.executeQuery(query);
			while (rs.next()) {
				lblDateA.setText(rs.getString("date"));
				lblDurationA.setText(rs.getString("duration"));
				lblRollNoA.setText(rs.getString("emp_id"));
