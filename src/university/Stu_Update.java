...
public class Stu_Update extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Stu_Update frame = new Stu_Update("");
	...
	public Stu_Update(String rollNo) {
		this.rollNo = rollNo;
		setTitle("Update Student");
		setIconImage(Toolkit.getDefaultToolkit().getImage(Stu_Update.class.getResource("/Images/uniLogo.png")));
		...
		JLabel lblHeading = new JLabel("UPDATE STUDENT DETAILS");
		...
		lblHeading.setBounds(222, 40, 304, 36);
		...
		btnUpdate.setBounds(318, 491, 107, 28);
		...
		JLabel lblRoll_No = new JLabel("Roll_No");
		lblRoll_No.setForeground(Color.WHITE);
		lblRoll_No.setFont(new Font("SansSerif", Font.BOLD | Font.ITALIC, 15));
		lblRoll_No.setBounds(70, 97, 62, 21);
		contentPane.add(lblRoll_No);

		lblRollAns = new JLabel("");
		lblRollAns.setForeground(Color.WHITE);
		lblRollAns.setFont(new Font("SansSerif", Font.BOLD | Font.ITALIC, 15));
		lblRollAns.setBounds(135, 97, 77, 21);
		contentPane.add(lblRollAns);
	...
	private void setText() {
		try {
			...
			String query = "select Roll_No,Student,Father_Name,Address,DateOfBirth,HSC_Per,Phone_No,CNIC,Batch from student where Roll_No  = '"
					+ rollNo + "'";
			ResultSet rs = con.statement.executeQuery(query);
			while (rs.next()) {
				lblRollAns.setText(rs.getString("Roll_No"));
				txtName.setText(rs.getString("Student"));
				txtFName.setText(rs.getString("Father_Name"));
				txtAddress.setText(rs.getString("Address"));
				((JTextField) dateChooser.getDateEditor().getUiComponent()).setText(rs.getString("DateOfBirth"));
				txtPercentage.setText(rs.getString("HSC_Per"));
	...
	private void updateBtn() {
		...
		try {
			...
			String query = "update student set student = '" + name + "', father_name ='" + fName + "', address = '"
					+ address + "', DateOfBirth = '" + dob + "',HSC_Per = '" + percent + "', Phone_No = '" + phone
					+ "',cnic = '" + cnic + "', batch = '" + batch + "', gender  = '" + gender + "', department = '"
					+ dept + "'where Roll_No = '" + rollNo + "' ";
