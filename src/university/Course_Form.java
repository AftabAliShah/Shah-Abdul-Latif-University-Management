...
public class Course_Form extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Course_Form frame = new Course_Form();
	...
	public Course_Form() {
		setTitle("Add Course");
		setIconImage(Toolkit.getDefaultToolkit().getImage(Course_Form.class.getResource("/Images/uniLogo.png")));
		...
		JLabel lblHeading = new JLabel("ADD COURSE");
		...
		lblHeading.setBounds(144, 40, 152, 36);
		...
		btnSubmit.setBounds(167, 471, 107, 28);
		...
		backBtn.setIcon(new ImageIcon(Course_Form.class.getResource("/Images/backSmall.png")));
		...
		comboDuration.setModel(new DefaultComboBoxModel(new String[] {"None", "1 Month", "1.5 Months", "2 Months", "2.5 Months", "3 Months", "3.5 Months", "4 Months"}));
		comboDuration.setForeground(Color.BLACK);
		comboDuration.setFont(new Font("SansSerif", Font.BOLD, 14));
		comboDuration.setBorder(new BevelBorder(BevelBorder.RAISED, new Color(0, 0, 0), new Color(0, 0, 0), null, null));
	...
	private void insertData() {
		try {
			...
			String query = "insert into Courses( Course_name,Course_Duration,Semester , Department , Course_type) "
					+ "values ('" + name + "','" + duration + "','" + semester + "','" + dept + "','" + type + "')";
